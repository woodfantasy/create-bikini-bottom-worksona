...
- 허용된 최대 대화 코퍼스에서 중복 없는 행동 신호 18–36개를 내부 장부로 정리한 뒤 공유용 증거 3개로 압축
- 캐릭터 후보, 매칭 점수, 신뢰도 제시
...
신뢰도를 높이려면 현재 호스트가 실제로 볼 수 있는 모든 턴과 사용자가 명시적으로 허용한 기록을 검토하도록 요청하세요. 여러 세션·주제·시기에 걸친 내부 증거 18–36개를 만들고 Agent 답변에 대한 수정·확인·거부도 포함합니다. 기록이 부족하면 대화 검색 권한을 주거나 대표 상호작용 단위 20–60개를 제공하세요. 원하면 낮은 신뢰도의 초안으로 진행할 수도 있습니다.

흐름은 ‘접근 가능한 코퍼스와 범위 보고 → 모든 턴 검토 또는 대규모 코퍼스 층화 표본 추출 → 행동 증거 18–36개 정리 → 캐릭터 매칭 → 공유용 증거 3개로 압축 → 카드 문구 작성 → 이미지·캡션 렌더링 → 크기·개인정보·권리·증거 범위 검토’입니다.
